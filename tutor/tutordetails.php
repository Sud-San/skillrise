...
SELECT 
	...
	td.institute_name
	FROM tutor_tbl t
	LEFT JOIN tutor_profile_tbl tp ON tp.tutor_id = t.tutor_id
	LEFT JOIN tutor_details td ON td.tutor_id = t.tutor_id
...
<body class="app">
                            ...
                            <div class="card shadow-sm border-0 rounded-4">
                                <div class="card-header bg-white pt-3 pb-3 border-bottom">
                                    <div class="row align-items-center">
                                        <div class="col">
                                            <h4 class="h3 mb-0">Tutor Details</h4>
                                        </div>
                                        <div class="col-auto">
                                            ...
                                            <tbody>
                                                <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <img src="<?= $row['profile_pic'] ?: 'assets/default-avatar.png'; ?>"
                                                        ...
                                                        <td class="text-start">
                                                            <div class="user-cell d-flex align-items-center gap-2">
                                                                ...
                                                                <i class="fa-solid fa-circle-info user-profile"
                                                                    data-id="<?= $row['tutor_id']; ?>"
                                                                    data-name="<?= htmlspecialchars($row['tutor_name'] ?? ''); ?>"
                                                                    data-location="<?= htmlspecialchars(($row['country'] ?? '')); ?>"
                                                                    data-email="<?= htmlspecialchars($row['tutor_email'] ?? ''); ?>"
                                                                    data-phone="<?= htmlspecialchars($row['tutor_phone'] ?? ''); ?>"
                                                                    data-language="<?= htmlspecialchars($row['languages_known'] ?? ''); ?>"
                                                                    data-degree="<?= htmlspecialchars($row['education'] ?? ''); ?>"
                                                                    data-courses='<?= json_encode(explode(",", $row["expertise"] ?? "")) ?>'
                                                                    data-certificates='<?= json_encode(explode(",", $row["achievements"] ?? "")); ?>'>
                                                                </i>
                                                        ...
                                                        <td>
                                                            <span
                                                                class="badge bg-<?= $row['verification_status'] == 'approved' ? 'success' : 'warning'; ?>">
                                                                <?= ucfirst($row['verification_status']); ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-center" data-status="<?= $row['tutor_status']; ?>">
                                                            <div class="d-flex flex-column align-items-center gap-1">
                                                                ...
                                                                <div class="form-check form-switch custom-switch">
                                                                    <input class="form-check-input status-switch"
                                                                        type="checkbox"
                                                                        id="flexSwitchCheck<?= $row['tutor_id']; ?>"
                                                                        <?= $row['tutor_status'] == 1 ? 'checked' : ''; ?>>
                                                                </div>

                                                                <!-- STATUS BADGE -->
                                                                <!--<?php if ($row['tutor_status'] == 1) { ?>
                ...
                <div class="profile-header">
                    <div class="d-flex align-items-center gap-4">
                        ...
                        <div class="flex-grow-1">
                            <div class="badge bg-light text-success mb-2 px-3 py-2 rounded-pill">Verified Tutor</div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('mouseenter', '.user-profile', function () {
                const el = $(this);

                clearTimeout(profileHoverTimeout);

                // Fill modal data
                $('#profileName').text(el.data('name'));
                $('#profileRole').text(el.data('role'));
                $('#profileLocation').text(el.data('location'));

                $('#infoEmail').text(el.data('email'));
                $('#infoPhone').text(el.data('phone'));
                $('#infoLanguage').text(el.data('language'));
                $('#infoDegree').text(el.data('degree'));

                $('#infoCourses').empty();
                (el.data('courses') || []).forEach(c =>
                    $('#infoCourses').append(`<li>${c}</li>`)
                );

                $('#infoCertificates').empty();
                (el.data('certificates') || []).forEach(c =>
            ...
            $(document).on('mouseleave', '.user-profile', function () {
                profileHoverTimeout = setTimeout(() => {
                    if (!$('#userProfileModal').is(':hover')) {
                        $('#userProfileModal').modal('hide');
                    }
                }, 300);
            });

            // Keep modal open if hovered
            $('#userProfileModal').on('mouseleave', function () {
                $('#userProfileModal').modal('hide');
            });
            ...
            $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
                ...
                let status = $(row).find('td[data-status]').attr('data-status');

                if (currentFilter === 'active' && status === '1') return true;
                if (currentFilter === 'inactive' && status === '0') return true;
            ...
            let table = $('#datatable').DataTable({
                ...
                initComplete: function () {
                    $('.filter-option[data-filter=""]').addClass('active');
                }
            });
            ...
            // CORRECTED: Filter option click handler
            $(document).on('click', '.filter-option', function (e) {
                e.preventDefault();

                currentFilter = $(this).data('filter'); // '', 'active', 'inactive'
                table.draw();
            });
            ...
            $(document).on('change', '.status-switch', function () {
                ...
                let statusTd = row.find('td[data-status]');

                // IMPORTANT: use attr(), NOT data()
                statusTd.attr('data-status', this.checked ? '1' : '0');
